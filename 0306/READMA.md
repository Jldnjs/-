...
- Aspect: 공통적인 관심사를 나타내는 모듈. 예를 들어 로깅, 보안, 트랜잭션 처리 등이 Aspect에 해당합니다.
...
- Aspect: 여러 객체에 공통으로 적용되는 기능을 Aspect라고 한다. 트랜잭션이나 보안 등이 Aspect의 좋은 예
 어드바이스와 포인트컷을 모듈화해 애플리케이션에 포함되는 횡단 기능.
...
# ProceedingJoinPoint 
: Spring AOP 프레임워크에서 제공하는 인터페이스. 현재 조인 포인트(join point)에 대한 정보와 조인 포인트를 진행하는 기능을 제공. 

 Around Advice에서 주로 사용됩니다. Around Advice는 조인 포인트 전후로 실행되는 Advice입니다. Around Advice는 ProceedingJoinPoint 인터페이스를 사용하여 조인 포인트에 대한 정보를 얻고 조인 포인트를 진행할지 여부를 결정할 수 있습니다.

- AOP(Aspect-Oriented Programming)는 코드를 모듈화하고 재사용성을 높이기 위해 관심사(concern)를 분리하는 프로그래밍 기법
- Join point는 AOP에서 코드 실행 지점을 의미합니다.
- Advice는 조인 포인트에 전후로 실행되는 코드입니다.
- ProceedingJoinPoint는 현재 조인 포인트에 대한 정보와 조인 포인트를 진행하는 기능을 제공하는 인터페이스입니다.

> 메서드
>   >- getArgs(): 현재 조인 포인트에 전달된 인수를 반환합니다.
>   >
>  >- getSignature(): 현재 조인 포인트의 메서드 시그니처를 반환합니다.
>   >
>  >- getTarget(): 현재 조인 포인트가 적용된 실제 타겟 객체를 반환합니다.
>   >
>  >- proceed(): 현재 조인 포인트를 진행합니다. 즉, 조인 포인트에 정의된 메서드를 실행합니다.
>   >

# org.springframework.jdbc.datasource.DataSourceTransactionManager
: Spring Framework에서 JDBC(Java Database Connectivity) 트랜잭션 관리를 위한 클래스. 
`트랜잭션 경계 설정, 커밋, 롤백 등 트랜잭션 관리 작업을 수행`

- JDBC DataSource 연결 관리: DataSource 객체를 통해 데이터베이스 연결을 설정하고 관리합니다.

- 트랜잭션 경계 설정: 트랜잭션 시작, 커밋, 롤백 시점을 결정합니다.

- 자동 트랜잭션 관리: 스프링의 트랜잭션 선언 어노테이션 (@Transactional)과 함께 사용하여 트랜잭션을 자동으로 관리합니다.

- 스레드 안전성: 여러 스레드에서 안전하게 사용할 수 있습니다.

`DataSourceTransactionManager 클래스는 스프링 AOP(Aspect-Oriented Programming)와 함께 사용될 수 있습니다. AOP를 사용하여 트랜잭션 관리를 메서드 호출 단위로 세분화할 수 있습니다.`
